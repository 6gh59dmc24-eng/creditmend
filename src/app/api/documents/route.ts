  ...
  try {
    ...
    if (!ALLOWED_FILE_TYPES.includes(file.type)) {
      return NextResponse.json({ 
        error: "Invalid file type. Only PDF, JPG, PNG, and Word documents are allowed." 
      }, { status: 400 })
    }
    ...
    // Save file metadata to database (encrypted sensitive info)
    const fileMetadata = {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filteredDocuments = category && category !== "all" 
